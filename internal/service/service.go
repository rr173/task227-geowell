...
func (svc *Service) CreateWellRun(code, name, well, logDate, unitTemp, unitPress string) (*model.WellRun, error) {
	...
	// idempotent: return existing run if code already claimed.
	if existing, err := svc.store.GetWellRunByCode(code); err == nil {
		if existing.Well != well || existing.UnitTemp != tu || existing.UnitPress != pu {
			return nil, fmt.Errorf("well run code %s already belongs to %s", code, existing.Well)
		}
		return existing, nil
	}
	r := &model.WellRun{
